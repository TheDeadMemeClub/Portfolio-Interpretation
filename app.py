...
def style_trend_cells(df, subset):
    """Pandas compatibility helper: Styler.applymap was removed/renamed in newer pandas.
    Use Styler.map when available, then fall back to applymap for older versions.
    """
    styler = df.style
    if hasattr(styler, "map"):
        return styler.map(trend_badge_color, subset=subset)
    return styler.applymap(trend_badge_color, subset=subset)
    ...
    if tech.empty:
        ...
        if not mtf_tech.empty:
            ...
            st.dataframe(
                ...
                .pipe(style_trend_cells, subset=tf_order),
